...
from agent.mdpr_rl import ActorR
...
class SaveAndLogCallback(BaseCallback):
    ...
    def _on_step(self) -> bool:
        ...
        if self.num_timesteps % self.save_every_steps == 0:
            path = Path(f"./ppo_step_{self.num_timesteps}.zip")
            self.model.save(str(path))
            if self.verbose > 0:
                print(f"[PPO] Saved checkpoint to {path}")

        ## Save actor networks weights as MaskDP
        ## Then just run the eval script on the checkpoints
        return True
...
def main(cfg):
    ...
    callback = SaveAndLogCallback(
        ...
        verbose=1,
        csv_name=f"ppo_train_log_{cfg.algorithm}.csv")
