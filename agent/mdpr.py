...
class MaskedDP(nn.Module):
    def __init__(self, obs_dim, action_dim, config, reward_dim=1):
        super().__init__()
        ...
        self.reward_head = nn.Sequential(
            nn.LayerNorm(self.n_embd),
            nn.ReLU(inplace=True),
            nn.Linear(self.n_embd, reward_dim),
        )
    ...
    def forward_encoder(self, states, actions, rewards=None, mask_ratio=0.):
    ...
    def forward_decoder(self, x, ids_restore):
        ...
        s = self.state_head(x[:, 0::3])
        a = self.action_head(x[:, 1::3])
        r = self.reward_head(x[:, 2::3])

        return s, a, r

    def forward_loss(self, target_s, target_a, pred_s, pred_a,
...
class MaskedDPAgent:
    ...
    def update_mdp(self, states, actions, rewards=None):
        metrics = dict()
